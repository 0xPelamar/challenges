### PortSwigger Lab: Blind SQL injection with time delays and information retrieval (Go Exploit)
This is a high-performance, concurrent exploitation tool written in Go to solve the PortSwigger Web Security Academy lab: [Blind SQL injection with time delays and information retrieval](https://portswigger.net/web-security/learning-paths/sql-injection/sql-injection-exploiting-blind-sql-injection-by-triggering-time-delays/sql-injection/blind/lab-time-delays-info-retrieval).

This tool brute-forces the administrator's password by injecting a PostgreSQL payload that causes the database to sleep if a character guess is correct.

The code injects the following logic into the cookie:
```
'%3b SELECT CASE WHEN (substring((select password from users where username='administrator'), $POS, 1)='$CHAR') THEN pg_sleep(5) ELSE pg_sleep(0) END --
